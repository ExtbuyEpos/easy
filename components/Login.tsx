...
import { Lock, User as UserIcon, AlertCircle, ChevronRight, Moon, Sun, LayoutDashboard, Loader2, Key, Store, Mail } from 'lucide-react';
...
export const Login: React.FC<LoginProps> = ({ 
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // Supreme Admin Identity Control
      const adminEmails = [
      ...
      setError(language === 'ar' ? 'فشل تسجيل الدخول' : 'Identity Verification Failed');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (method === 'CREDENTIALS') {
      ...
      // Data Isolation: Find the vendor by their specific Vendor Code
      const targetVendor = users.find(u => 
        u.role === 'VENDOR' && u.vendorId?.toLowerCase() === vendorCodeInput.trim().toLowerCase()
      );

      if (!targetVendor) {
        setError(language === 'ar' ? 'رمز المورد غير معروف' : 'Vendor Code not recognized');
        setLoading(false);
        return;
      }

      // Check the access code assigned specifically to this vendor
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] dark:bg-black flex flex-col md:items-center md:justify-center p-0 transition-colors font-sans">
      ...
      <div className="fixed top-8 right-8 flex gap-3 z-[100]">
         <button onClick={toggleTheme} className="p-3 bg-white/10 backdrop-blur-2xl rounded-2xl text-white hover:bg-white/20 transition-all border border-white/10 shadow-2xl">
            {isDarkMode ? <Sun size={20} /> : <Moon size={20} />}
         </button>
         <button onClick={toggleLanguage} className="px-5 py-3 bg-white/10 backdrop-blur-2xl rounded-2xl text-white hover:bg-white/20 transition-all font-black text-[10px] uppercase tracking-widest border border-white/10 shadow-2xl">
            {language === 'en' ? 'Arabic' : 'English'}
         </button>
      </div>

      <div className="bg-white dark:bg-slate-900 md:rounded-[4rem] shadow-2xl overflow-hidden w-full max-w-6xl flex flex-col md:flex-row animate-fade-in md:h-[min(860px,95vh)] relative border border-white/5">
              ...
              {method === 'VISITOR_CODE' ? (
                ...
                <div className="space-y-12 animate-fade-in text-center py-6">
                    <div className="py-16 px-10 bg-slate-50 dark:bg-slate-800/30 rounded-[4rem] border-2 border-dashed border-slate-200 dark:border-slate-800">
                        ...
                        <p className="text-xs text-slate-400 font-medium mt-3 leading-relaxed max-w-[240px] mx-auto uppercase">Cloud-based verification node.</p>
                    </div>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between items-center px-6">
                        ...
                        <button type="button" className="text-[9px] font-black text-[#0ea5e9] uppercase tracking-widest hover:underline italic">RESET KEYS</button>
                      </div>
                      <div className="relative group">
