...
import { Lock, User as UserIcon, AlertCircle, ChevronRight, Moon, Sun, LayoutDashboard, Loader2, Key, Store, Mail, Phone } from 'lucide-react';
...
export const Login: React.FC<LoginProps> = ({ 
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      const isSystemAdmin = adminEmails.includes(fbUser.email?.toLowerCase() || '');
      onLogin({
        id: fbUser.uid,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (method === 'CREDENTIALS') {
      ...
      // Find the specific vendor by the entered Vendor Code
      const targetVendor = users.find(u => 
        u.role === 'VENDOR' && u.vendorId?.toLowerCase() === vendorCodeInput.trim().toLowerCase()
      );

      if (!targetVendor) {
        setError(t('vendorNotFound') || 'Vendor Code not found.');
        setLoading(false);
        return;
      }

      // Check the access code for that specific vendor
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] dark:bg-black flex flex-col md:items-center md:justify-center p-0 transition-colors font-sans">
      ...
      <div className="fixed top-8 right-8 flex gap-3 z-[100]">
         <button onClick={toggleTheme} className="p-3.5 bg-white/5 backdrop-blur-2xl rounded-2xl text-white hover:bg-white/10 transition-all border border-white/10 shadow-xl">
            {isDarkMode ? <Sun size={20} /> : <Moon size={20} />}
         </button>
         <button onClick={toggleLanguage} className="px-6 py-3.5 bg-white/5 backdrop-blur-2xl rounded-2xl text-white hover:bg-white/10 transition-all font-bold text-[10px] uppercase tracking-widest border border-white/10 shadow-xl">
            {language === 'en' ? 'Arabic' : 'English'}
         </button>
      </div>

      <div className="bg-white dark:bg-slate-900 md:rounded-[4rem] shadow-2xl overflow-hidden w-full max-w-6xl flex flex-col md:flex-row animate-fade-in md:h-[min(880px,96vh)] relative border border-white/5">
        
        {/* Brand Left Panel (Matches Screenshot) */}
        <div className="bg-[#0ea5e9] p-12 md:p-24 md:w-[45%] flex flex-col justify-center text-white relative overflow-hidden shrink-0">
          <div className="relative z-10 space-y-16">
            <div className="flex items-center gap-6">
              <div className="bg-white p-5 rounded-[2rem] text-[#0ea5e9] shadow-2xl rotate-12 transform hover:rotate-0 transition-all duration-500 cursor-pointer">
          ...
          <div className="absolute bottom-[-10%] left-[-10%] w-[400px] h-[400px] bg-brand-400/20 rounded-full blur-[80px]"></div>
        </div>

        {/* Right Panel: Login Core */}
          ...
          <div className="max-w-md mx-auto w-full space-y-12">
              ...
              {method === 'VISITOR_CODE' ? (
                <form onSubmit={handleSubmit} className="space-y-10 animate-fade-in-up">
                    <div className="p-10 bg-slate-50/50 dark:bg-slate-800/30 rounded-[3.5rem] border border-slate-200/50 dark:border-slate-800 flex flex-col items-center text-center gap-5">
                        <div className="w-20 h-20 bg-white dark:bg-slate-800 rounded-[1.8rem] flex items-center justify-center shadow-2xl text-[#0ea5e9] border border-slate-100">
                           <Store size={40} />
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="relative group">
                                <input 
                                    ...
                                    className="w-full p-6 bg-slate-100/50 dark:bg-slate-800 border-2 border-transparent focus:border-[#0ea5e9] rounded-[2.5rem] outline-none transition-all dark:text-white font-bold text-lg pl-16 shadow-inner" 
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="relative group">
                                <input 
                                    ...
                                    className="w-full p-6 bg-slate-100/50 dark:bg-slate-800 border-2 border-transparent focus:border-[#0ea5e9] rounded-[2.5rem] outline-none transition-all dark:text-white font-black text-3xl tracking-[0.4em] pl-16 shadow-inner" 
                                    placeholder="••••" 
                                    required
                                />
                    ...
                    <button type="submit" disabled={loading || !visitorCode || !vendorCodeInput} className="w-full py-7 bg-gradient-to-r from-[#38bdf8] to-[#0ea5e9] text-white rounded-[2.5rem] font-black uppercase tracking-[0.2em] text-xs shadow-2xl active:scale-[0.98] transition-all flex items-center justify-center gap-4 italic group">
                ...
                <div className="space-y-12 animate-fade-in text-center py-6">
                    <div className="py-16 px-10 bg-slate-50 dark:bg-slate-800/30 rounded-[4rem] border-2 border-dashed border-slate-200 dark:border-slate-800">
                        ...
                        <p className="text-xs text-slate-400 font-medium mt-3 leading-relaxed max-w-[240px] mx-auto uppercase">Secure multi-factor authentication via Google Cloud integration.</p>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-10 animate-fade-in py-4">
                  <div className="space-y-6">
                    <div className="space-y-2">
                      ...
                      <div className="relative group">
                        <UserIcon className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-300" size={22} />
                        <input 
                            type="text" 
                            value={username} 
                            onChange={e => setUsername(e.target.value)} 
                            className="w-full pl-16 pr-6 py-6 bg-slate-100/50 dark:bg-slate-800 border-2 border-transparent focus:border-[#0ea5e9] rounded-[2.5rem] outline-none font-bold dark:text-white text-lg shadow-inner" 
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between items-center px-6">
                        ...
                        <button type="button" className="text-[9px] font-black text-[#0ea5e9] uppercase tracking-widest hover:underline italic">RECOVER ACCESS</button>
                      </div>
                      <div className="relative group">
