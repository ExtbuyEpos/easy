                ...
                <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                    {users.map(u => (
                        <tr key={u.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-all">
                            ...
                            <td className="p-10 font-black text-slate-900 dark:text-white"><div className="flex items-center gap-2">{u.name} {u.role === 'ADMIN' && <ShieldCheck size={14} className="text-emerald-500" />}</div><span className="block text-[9px] text-slate-400 uppercase tracking-widest mt-1">@{u.username}</span></td>
