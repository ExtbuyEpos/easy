...
import { Trash2, Plus, X, Receipt, ChevronLeft, Users, Zap, Globe, Heart, Database, Cloud, Bell, ShieldCheck, Share2, Clipboard, BarChart, MessageSquare, ExternalLink, RefreshCw, CheckCircle2, Upload, Image as ImageIcon, Key, LogOut, ShieldEllipsis, ShieldAlert, Edit2, Lock } from 'lucide-react';
...
export const Settings: React.FC<SettingsProps> = ({ 
  ...
  // SYSTEM OWNER PROTOCOL
  const SYSTEM_OWNER_ID = 'admin_1';
  const SYSTEM_OWNER_EMAIL = 'zahratalsawsen1@gmail.com';
  const isSystemOwner = currentUser.id === SYSTEM_OWNER_ID || currentUser.email?.toLowerCase() === SYSTEM_OWNER_EMAIL;
  ...
  const handleUserSubmit = () => {
      ...
      if (editingOperatorId) {
          ...
          // Rule: System Owner only change password for protected/admin nodes
          const isTargetAdmin = targetUser.role === 'ADMIN';
          const isSelf = targetUser.id === currentUser.id;
          
          let finalPassword = targetUser.password;
          if (userFormData.password !== targetUser.password) {
              // Someone is trying to change a password
              if (isTargetAdmin && !isSystemOwner && !isSelf) {
                  alert("SECURITY: Only the System Owner can reset other Administrator passwords.");
                  return;
              }
              finalPassword = userFormData.password;
          }
                ...
                <div className="relative z-10 space-y-6">
                    <div className="space-y-3">
                        ...
                        <div className="relative group max-w-md">
                            <input type="text" value={storeForm.visitorAccessCode || ''} onChange={e => setStoreForm({...storeForm, visitorAccessCode: e.target.value})} className="w-full p-5 bg-black/40 border-2 border-white/10 group-focus-within:border-brand-500 rounded-3xl font-black text-white text-2xl tracking-[0.3em] outline-none transition-all pl-14" placeholder="e.g. 2026" />
                            <Key className="absolute left-5 top-1/2 -translate-y-1/2 text-brand-500" size={24} />
                        </div>
                        <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest italic leading-relaxed">Set the master security code for visitors entering the virtual showroom.</p>
                    </div>
                    ...
                    <div className="flex flex-col md:flex-row gap-6 items-center">
                        <div className="w-32 h-32 rounded-[2.5rem] bg-slate-50 dark:bg-slate-800 border-2 border-dashed border-slate-200 dark:border-slate-700 flex items-center justify-center overflow-hidden shrink-0 shadow-inner group relative">
                            {storeForm.logo ? (
                                ...
                                <div className="text-slate-300 dark:text-slate-600 flex flex-col items-center gap-1"><ImageIcon size={32} strokeWidth={1.5} /><span className="text-[8px] font-black uppercase tracking-tighter">Branding Layer</span></div>
                            )}
                        </div>
                        <div className="flex-1 space-y-3 w-full">
                            <button onClick={() => fileInputRef.current?.click()} className="w-full py-4 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-2xl border border-slate-200 dark:border-slate-700 transition-all font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-3"><Upload size={16} /> {t('uploadLogo')}</button>
                    ...
                    {users.map(u => {
                        ...
                        return (
                            <tr key={u.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-all">
                                <td className="p-10 font-mono text-xs text-slate-400 font-black">{u.employeeId ? formatNumber(u.employeeId.split('-')[1] || u.employeeId, language) : '٠'}</td>
                                <td className="p-10 font-black text-slate-900 dark:text-white"><div className="flex items-center gap-2">{u.name} {isProtected && <ShieldCheck size={14} className="text-emerald-500" title="System Protected" />}{isSelf && <span className="text-[8px] bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 rounded uppercase">You</span>}</div><span className="block text-[9px] text-slate-400 uppercase tracking-widest mt-1">@{u.username}</span></td>
                ...
                <div className="p-10 space-y-6 overflow-y-auto max-h-[70vh] custom-scrollbar">
                    ...
                    <div className="space-y-1">
                        ...
                        <input 
                            ...
                            className="w-full p-4 bg-slate-50 dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 rounded-2xl font-bold dark:text-white" 
                            placeholder="••••••••"
                            disabled={editingOperatorId && users.find(u => u.id === editingOperatorId)?.role === 'ADMIN' && !isSystemOwner && editingOperatorId !== currentUser.id}
                        />
                    </div>
                </div>
                <div className="p-10 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-100 dark:border-slate-800 flex gap-4"><button onClick={() => setIsModalOpen(false)} className="flex-1 py-5 text-slate-500 font-black uppercase tracking-widest text-[10px]">{t('cancel')}</button><button onClick={handleUserSubmit} className="flex-[2] py-5 bg-brand-600 text-white font-black uppercase tracking-widest text-[10px] rounded-[2rem] shadow-xl hover:bg-brand-500 transition-all">{editingOperatorId ? 'Update Identity' : t('authorizeOperator')}</button></div>
            </div>
        </div>
      )}
    </div>
