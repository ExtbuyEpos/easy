...
import { TrendingUp, Sparkles, PieChart, FileText, ChevronLeft, Activity, Target, History, ClipboardList, Loader2, FileSpreadsheet, Calendar as CalendarIcon, ArrowRight, Download, Package, ShoppingBag, BarChart3, Users, Zap } from 'lucide-react';
...
export const Reports: React.FC<ReportsProps> = ({ sales, products, onGoBack, language, users = [] }) => {
  ...
  const stats = useMemo(() => {
    const filtered = sales.filter(s => {
      ...
      if (dateRange === 'today') return s.timestamp >= today;
      if (dateRange === 'yesterday') return s.timestamp >= yesterday && s.timestamp < today;
    ...
    filtered.forEach(sale => {
      ...
      if (!operatorPerformance[opId]) {
        ...
        operatorPerformance[opId] = { name: user?.name || 'External System', sales: 0, count: 0 };
      }
      ...
      <div className="flex flex-col gap-6 mb-8 shrink-0">
        <div className="flex flex-col md:flex-row justify-between md:items-center gap-6">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h2 className="text-xl md:text-3xl font-black text-slate-900 dark:text-white tracking-tighter uppercase italic leading-none">Intelligence Engine</h2>
                    <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.3em] opacity-60 mt-1">Audit Control & BI Performance</p>
                </div>
            </div>
            <div className="flex bg-white dark:bg-slate-900 rounded-[1.8rem] p-1.5 shadow-xl border border-slate-200 dark:border-slate-800">
                ...
                <button onClick={() => setActiveTab('PRODUCTS')} className={`px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'PRODUCTS' ? 'bg-brand-600 text-white shadow-lg' : 'text-slate-400'}`}>Products</button>
                <button onClick={() => setActiveTab('OPERATORS')} className={`px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'OPERATORS' ? 'bg-brand-600 text-white shadow-lg' : 'text-slate-400'}`}>Staff BI</button>
            </div>
        </div>
        ...
        <div className="flex flex-col xl:flex-row justify-between gap-4 items-start md:items-center">
            ...
            <div className="flex gap-3">
               <button onClick={handleExportZReport} className="px-6 py-3 bg-slate-900 text-white rounded-2xl font-black text-[9px] uppercase tracking-widest flex items-center gap-2 shadow-lg active:scale-95 transition-all"><FileText size={16}/> Print Z-Report</button>
               <button onClick={handleExportProcurementList} className="px-6 py-3 bg-brand-600 text-white rounded-2xl font-black text-[9px] uppercase tracking-widest flex items-center gap-2 shadow-lg active:scale-95 transition-all"><Package size={16}/> Procurement List</button>
            </div>
        </div>
      </div>
        ...
        {activeTab === 'FINANCIAL' ? (
          <div className="space-y-8 animate-fade-in">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                <div className="lg:col-span-8 bg-slate-900 rounded-[3.5rem] p-10 md:p-14 text-white relative overflow-hidden shadow-2xl flex flex-col justify-center min-h-[300px] group">
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-12 relative z-10">
                        ...
                        <div className="space-y-3">
                            <span className="text-[10px] font-black text-brand-400 uppercase tracking-widest block">Period Profit</span>
                        ...
                        <div className="space-y-3">
                            <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest block">Transactions</span>
                            <span className="text-4xl font-black text-white">{stats.transactions}</span>
                        </div>
                    </div>
                </div>
                <div className="lg:col-span-4 bg-brand-600 rounded-[3.5rem] p-12 text-white relative overflow-hidden shadow-2xl flex flex-col justify-between group">
                    <div>
                        <div className="flex items-center gap-3 mb-8"><PieChart size={24}/><h3 className="text-[11px] font-black uppercase tracking-widest text-brand-100">AI Intelligent Core</h3></div>
                        <p className="text-lg font-medium italic opacity-95 leading-relaxed line-clamp-4">{insight || "Initiate a Gemini AI deep-scan to identify sales anomalies and market opportunities for this period."}</p>
                    </div>
                    <button onClick={handleGenerateInsight} disabled={loadingAi} className="bg-white text-brand-600 w-full py-6 rounded-3xl font-black text-[11px] uppercase tracking-widest shadow-2xl active:scale-95 disabled:opacity-50 mt-12 flex items-center justify-center gap-3 italic transition-all">
                        {loadingAi ? <Loader2 size={20} className="animate-spin" /> : <Sparkles size={18}/>} {loadingAi ? 'Scanning...' : 'Execute Deep Scan'}
                    </button>
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-[3.5rem] shadow-2xl border border-slate-100 dark:border-slate-800 overflow-hidden">
                <div className="p-8 border-b border-slate-50 dark:border-slate-800 flex flex-col md:flex-row justify-between items-center gap-4 bg-slate-50/30 dark:bg-slate-900/10">
                    <div>
                      ...
                      <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">Audit log for {dateRange} operations</p>
                    </div>
                    <div className="flex gap-3">
                        <div className="flex items-center gap-6 px-8 py-3 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700">
                            <div className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Methods:</div>
                            <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-emerald-500"></div><span className="text-[10px] font-black text-slate-700 dark:text-slate-300">CASH {formatCurrency(stats.cashTotal, language, CURRENCY)}</span></div>
                            <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-brand-500"></div><span className="text-[10px] font-black text-slate-700 dark:text-slate-300">CARD {formatCurrency(stats.cardTotal, language, CURRENCY)}</span></div>
                        </div>
                    </div>
                </div>
                  ...
                  <table className="w-full text-left border-separate border-spacing-0">
                      <thead className="bg-slate-50 dark:bg-slate-800 text-slate-400 font-black uppercase text-[9px] tracking-widest sticky top-0 z-10 backdrop-blur-md">
                        <tr>
                          ...
                          <th className="p-10">Customer Context</th>
                          <th className="p-10">Ref ID</th>
                                ...
                                <tr key={s.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-all group">
                                    ...
                                    <td className="p-10">
                                        {s.customerName ? (
                                            ...
                                            <span className="text-[10px] font-black text-slate-300 uppercase italic">Walking Customer</span>
                                        )}
                                    </td>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-fade-in-up">
              <div className="bg-white dark:bg-slate-900 rounded-[3rem] p-10 border border-slate-100 dark:border-slate-800 shadow-2xl">
                  <div className="flex items-center gap-3 mb-10"><ShoppingBag size={24} className="text-brand-500"/><h3 className="text-xl font-black italic uppercase tracking-tighter dark:text-white">Top Sales performance</h3></div>
              ...
              <div className="bg-white dark:bg-slate-900 rounded-[3rem] p-10 border border-slate-100 dark:border-slate-800 shadow-2xl">
                  <div className="flex items-center gap-3 mb-10"><PieChart size={24} className="text-brand-500"/><h3 className="text-xl font-black italic uppercase tracking-tighter dark:text-white">Contribution Mix</h3></div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[3.5rem] shadow-xl border border-slate-100 dark:border-slate-800 overflow-hidden animate-fade-in-left">
              <div className="p-10 border-b border-slate-50 dark:border-slate-800 bg-slate-50/30 flex justify-between items-center">
                  <div className="flex items-center gap-4">
                      ...
                      <div>
                          <h4 className="text-2xl font-black italic uppercase tracking-tighter dark:text-white">Operator efficiency Matrix</h4>
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Employee performance audit trail</p>
                      </div>
                  </div>
              </div>
              <div className="overflow-x-auto">
                  <table className="w-full text-left border-separate border-spacing-0">
                      <thead className="bg-slate-50 dark:bg-slate-800 text-slate-400 font-black uppercase text-[10px] tracking-widest sticky top-0 z-10">
                          <tr>
                              ...
                              <th className="p-10 text-center">Transaction Count</th>
                              <th className="p-10 text-right">Revenue Share</th>
                              <th className="p-10 text-right">Efficiency Ratio</th>
                          </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                          {stats.operators.map((op, idx) => (
                              <tr key={idx} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-all">
                                  <td className="p-10">
                                      <div className="flex items-center gap-4">
                                          <div className="w-12 h-12 bg-brand-600 rounded-2xl flex items-center justify-center font-black text-white text-lg shadow-xl">{op.name.charAt(0)}</div>
                                  ...
                                  <td className="p-10 text-right">
                                      <span className="px-4 py-2 bg-slate-100 dark:bg-slate-800 rounded-xl text-[10px] font-black uppercase text-slate-500">
                                          {formatNumber((op.sales / stats.revenue) * 100, language)}% Total Share
                                      </span>
                                  </td>
                              </tr>
                          ))}
